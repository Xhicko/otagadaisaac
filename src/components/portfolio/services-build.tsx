...
const buildServiceIndexes = [0, 1, 2, 8, 9];

export function ServicesBuild() {
  const [showDetails, setShowDetails] = useState(false);
  const [showAll, setShowAll] = useState(false);
  const buildServices = buildServiceIndexes.map((index) => services[index]);
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto text-center">
          ...
          <h2 className="mt-3 text-3xl font-semibold tracking-tight text-[#0F172A] sm:text-4xl">
            Full-stack systems clients can hire Isaac to build, Support and Launch
          </h2>
          <p className="mt-6 w-full text-base leading-7 text-[#334155] sm:text-lg">
            Application infrastructure for portals, admin operations, payments,
            AI knowledge tools, and workflow records.
          </p>
        </div>

        <p className="mx-auto mt-10 w-fit text-xs font-semibold uppercase tracking-[0.16em] text-[#64748B]">
          Service - Build
        </p>
        ...
        <ol className="mx-auto mt-12 max-w-6xl">
          {buildServices.map((service, index) => {
            const isFadedPreview = index === 2;
            const isHiddenUntilExpanded = index > 2;
        ...
        <div className="flex justify-center">
          ...
          >
            ...
            />
            <span>{showAll ? "Less, Please" : "More Build Services"}</span>
          </button>
        </div>
      </div>
    </section>
