...
const launchServiceIndexes = [3, 4, 5, 6, 7];

export function ServicesLaunch() {
  const [showDetails, setShowDetails] = useState(false);
  const [showAll, setShowAll] = useState(false);
  const launchServices = launchServiceIndexes.map((index) => services[index]);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex justify-center">
          ...
          >
            ...
            />
            <span>{showAll ? "Less, Please" : "More Launch Services"}</span>
          </button>
        </div>
      </div>
    </section>
